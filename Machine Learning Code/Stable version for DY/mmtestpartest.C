...
void mmtestpartest(Int_t ntrain = 200) {
...
//--type signal and background: 1-signal, 0-background
   ...
   //-- train NN
   TMultiLayerPerceptron *mlp = new TMultiLayerPerceptron("@pt1,@eta1,@phi1,@pt2,@eta2,@phi2,@Nak8j,@missET:14:8:5:3:type",simu,"Entry$%2", "(Entry$+2)%2"); 
...
//--feeding to NN
   ...
   MLPmm-> Scale(100/MLPmm->GetEntries());
   MLPmmb-> Scale(100/MLPmmb->GetEntries());
