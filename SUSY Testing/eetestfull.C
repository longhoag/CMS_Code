//--best testing yet
//--not all input(missing vx,vy,nj), wo diagrams, ee branches, unify variables, all data sets, normalize
...
void eetestfull(Int_t ntrain = 200) {
   const char* fname = "group_SUSY_1100_100mm.root";
   const char* fname2 = "group_TT.root";
   const char* fname3 = "group_DY.root";
   const char* fname4 = "group_WW.root";
   const char* fname5 = "group_WZ.root";
   const char* fname6 = "group_ZZ.root";
...
//
   ...
   TTree* eeb5_Branch = (TTree*) input6->Get("ee_Branch");
...
//-- set branches
   ...
   eeb5_Branch->SetBranchAddress("pt1",  &pt1_ee);
   eeb5_Branch->SetBranchAddress("eta1", &eta1_ee);
   eeb5_Branch->SetBranchAddress("phi1", &phi1_ee);
   eeb5_Branch->SetBranchAddress("pt2",  &pt2_ee);
   eeb5_Branch->SetBranchAddress("eta2", &eta2_ee);
   eeb5_Branch->SetBranchAddress("phi2", &phi2_ee);
   eeb5_Branch->SetBranchAddress("Nak8j",&Nak8j_ee);
   eeb5_Branch->SetBranchAddress("missET",&missET_ee);
//  
...
//--type signal and background: 1-signal, 0-background
   ...
   type = 0;
   for(i = 0; i < eeb5_Branch->GetEntries(); i++) {
      eeb5_Branch->GetEntry(i);
      simu->Fill();
   }
   ...
   mlpa_canvas->cd(4);
   TH1F* MLPee = new TH1F("MLPeeh", "NN output (ee-susy(1)-all(0))", 50, -.5, 1.5);
   TH1F* MLPeeb = new TH1F("MLPeebh", "NN output (ee-susy(1)-all(0))", 50, -.5, 1.5);
